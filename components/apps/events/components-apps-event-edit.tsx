...
const ComponentsAppsEventEdit = () => {
    ...
    useEffect(() => {
        const fetchEvent = async () => {
            ...
            try {
                ...
                const event = data.events.find((e: Event) => e._id === eventId);
                ...
                if (event) {
                    setFormData({
                        ...
                        paragraphs: '', // Initialize as empty, will be populated if event has paragraphs
                        displayDate: new Date(event.displayDate).toISOString().slice(0, 16), // Format for datetime-local
                    ...
                    if (event.image) {
                        setImages([{ dataURL: event.image, file: null }]);
                    }
                } else {
        ...
        fetchEvent();
    }, [session?.accessToken, eventId, router]);
    ...
    const validateForm = (): boolean => {
        ...
        if (!formData.displayDate) newErrors.displayDate = 'Display date is required';
        ...
        return (
            <div className="panel">
                <div className="text-center py-8">
                    ...
                    <p className="mt-4 text-gray-500">Loading event data...</p>
                </div>
            </div>
        );
    }
...
export default ComponentsAppsEventEdit;
